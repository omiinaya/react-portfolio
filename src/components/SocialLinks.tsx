...
const SocialLinks: React.FC = () => {
  ...
  return (
    <div className="social-links">
            ...
            onClick={() => {
              if (!isDevelopmentMode()) {
                trackSocialMediaClick(
                  profileItem.network,
                  profileItem.url.href,
                );
              }
            }}
          >
            <IconComponent />
            <span className="social-network">{profileItem.network}</span>
          </a>
        );
      })}
      <a
      ...
      >
        <MailIcon />
        <span className="social-network">Email</span>
      </a>
    </div>
  );
};

// SVG Icons for Social Media (reused from Footer)
const GitHubIcon = () => (
  <svg
    width="24"
    height="24"
