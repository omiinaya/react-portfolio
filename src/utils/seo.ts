...
export const generatePersonStructuredData = (profile: any): StructuredData => {
  ...
  return {
    ...
    description:
      ...
      `${basics.headline} based in ${basics.location}`,
    ...
    sameAs:
      sections.profiles?.items
        ?.map((profile: any) => profile.url?.href)
        ?.filter(Boolean) || [],
...
export const generatePortfolioStructuredData = (
  ...
  return {
    ...
    description:
      sections.summary?.content ||
      `Portfolio of ${basics.name}, ${basics.headline}`,
    author: {
      "@type": "Person",
      name: basics.name,
    },
 ...
 */
export const generateProjectStructuredData = (
  project: any,
  author: string,
): StructuredData => {
  return {
 ...
 */
export const generateCertificationStructuredData = (
  certification: any,
): StructuredData => {
  return {
    "@context": "https://schema.org",
    ...
    credentialCategory: certification.issuer?.includes("Comptia")
      ? " Certification"
      : "Educational",
...
export const generateMetaTags = (seoData: SEOData): any[] => {
  ...
  const metaTags = [
    ...
    { name: "description", content: description },
    {
      ...
      content:
        keywords?.join(", ") ||
        "portfolio, developer, full stack, react, javascript, typescript",
    },
